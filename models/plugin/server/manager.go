...
func (m *Manager) Login(content *LoginContent) (*LoginContent, error) {
	...
	for _, p := range m.loginPlugins {
		res, retContent, err = p.Handle(OpLogin, *content)
		if err != nil {
			log.Warn("send Login request to plugin [%s] error: %v", p.Name(), err)
...
func (m *Manager) NewProxy(content *NewProxyContent) (*NewProxyContent, error) {
	...
	for _, p := range m.newProxyPlugins {
		res, retContent, err = p.Handle(OpNewProxy, *content)
		if err != nil {
			log.Warn("send NewProxy request to plugin [%s] error: %v", p.Name(), err)
